...
vector<shared_ptr<Bunny>> bunnies; // !C++11: vector<Bunny*> bunnies;
Program prog;
Camera camera;
...
void loadScene()
{
	...
	int n = 100;
	for(int i = 0; i < n; ++i) {
		auto bunny = make_shared<Bunny>(); // !C++11: Bunny *bunny = new Bunny();
...
void initGL()
{
	...
	// !C++11
	//	for(vector<Bunny*>::iterator p = bunnies.begin(); p != bunnies.end(); ++p) {
	//		(*p)->init(&prog);
	//	}
...
// Sort bunnies by their z values in camera space
class BunnySorter {
public:
	// !C++11 bool operator()(const Bunny *p0, const Bunny *p1) const
	bool operator()(const shared_ptr<Bunny> p0, const shared_ptr<Bunny> p1) const
	{
		// Bunny positions in world space
		const glm::vec3 &x0 = p0->getPosition();
		const glm::vec3 &x1 = p1->getPosition();
		// Bunny positions in camera space
